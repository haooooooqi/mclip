...
JOBNAME=flax/$(date +%Y%m%d_%H%M%S)_${VM_NAME}_${CONFIG}_maetf_${ep}ep_b${batch}_lr${lr}_mask${mask}_TorchLoader_wseed${seed}_patchaug_re${repeats}
RESUME_DIR=''
...
python3 main.py \
    ...
    --config.seed_pt=${seed} \
    --config.seed_tf=${seed} \
    --config.aug.patch_aug.repeats=${repeats} \

...
# lr factor func
def lrf_func(keys: List, lr_factor: float):
    """Get the lrf function."""
    return functools.partial(_lr_factor, keys=keys, lr_factor=lr_factor)


def _lr_factor(
        path: Tuple[Any], val: jnp.ndarray,
        keys: List, lr_factor: float):
    """Get the layerwise lr decay rate based on name."""
    del val

...
def _decode_and_center_crop_png_pad(image_bytes, image_size, **kwargs):
  """Crops to center of image with padding then scales image_size."""
...
def _decode_and_center_crop_png(image_bytes, image_size, **kwargs):
  """Crops to center of image with padding then scales image_size."""
  image = tf.io.decode_png(image_bytes, channels=3)
  image = tf.compat.v1.image.resize_bicubic([image], [image_size, image_size])[0]  # TF MAE: tf.image.resize_bicubic
  return image
...
def _decode_and_center_crop_f(image_bytes, image_size, **kwargs):
  """Crops to center of image with padding then scales image_size."""
  try:
...
decode_and_random_crop ={
    ...
    # note the following transform is experimental, should not be the final version
    'vc_f': _decode_and_center_crop_f,
    'vc_png_pad': _decode_and_center_crop_png_pad,
    'vc_png_re': _decode_and_center_crop_png,
  }

...
from t5x.train_state_initializer_ft import create_train_state
...
def compute_eval_metrics(logits, labels, labels_one_hot):
    ...
    metrics = {"loss": loss, "accuracy": accuracy, "label": labels}
...
def train_step(state, batch, model, rng):
    ...
    new_state = state.apply_gradient(grads, learning_rate=None, flax_mutables=new_mutables)  # TODO: not used in adamw
...
def train_and_evaluate(config: ml_collections.ConfigDict, workdir: str) -> train_state.TrainState:
    ...
    # Create checkpointer
    # ------------------------------------
    # allow redirecting the checkpoint to another directory with "T5X_CKPT_DIR" env variable
    checkpoints_dir = os.environ.get("T5X_CKPT_DIR", workdir)
    checkpointer = t5x.checkpoints.Checkpointer(
        train_state=state_shape, partitioner=partitioner, checkpoints_dir=checkpoints_dir, keep=None
    )
    ...
    partitioned_eval_step = partitioner.partition(
        eval_step_fn, in_axis_resources=(state_axes, partitioner.data_partition_spec), out_axis_resources=None
    )
    ...
    for epoch in range(epoch_offset, int(config.num_epochs)):
        ...
        for i, batch in enumerate(data_loader_train):
            ...
            if config.get("log_every_steps"):
                ...
                if (step + 1) % config.log_every_steps == 0:
                    ...
                    summary = {
                        f"train_{k}": float(v) for k, v in jax.tree_map(lambda x: x.mean(), train_metrics).items()
                    }
        ...
        if (epoch + 1) % config.save_every_epochs == 0 or epoch + 1 == int(config.num_epochs):
            logging.info("Saving checkpoint: {}".format(checkpoints_dir))
...
def run_eval(state, partitioned_eval_step, data_loader_val, local_batch_size, epoch):
    ...
    logging.info(
        "Eval time: {}, {} steps, {} samples".format(
            str(datetime.timedelta(seconds=int(toc))), len(data_loader_val), len(eval_metrics["test_acc1"])
        )
    )

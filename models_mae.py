...
from models_vit import ClipVisualBackbone, ClipTextualBackbone
...
def random_mask(rng, x, mask_ratio, bias=None):
  ...
  return x_masked, mask, ids_restore, ids_keep, ids_dump
...
class LanguageTransformer(nn.Module):
  ...
  def apply_encoder(self, inputs, train):
    ...
    # x, mask, ids_restore, _, _ = random_mask(self.make_rng('dropout'), x, self.mask_ratio)
...
class VisionTransformer(nn.Module):
  ...
  def compute_loss(self, imgs, pred, mask, teacher=None):
    """
    imgs: [N, H, W, 3]
    pred: [N, L, p*p*3]
    ...
    if teacher is None :
      ...
      # teacher = teacher / teacher.norm(dim=2, keepdim=True)
      # assert pred.shape == teacher.shape
      # loss = 2 - 2 * (pred * teacher).sum(dim=-1)
      # loss = loss.mean()
      # print(pred.shape, teacher.shape)
      pred = pred / jnp.linalg.norm(pred, axis=2, keepdims=True)
      teacher = teacher / jnp.linalg.norm(teacher, axis=2, keepdims=True)

      assert pred.shape == teacher.shape, f"{pred.shape}, {teacher.shape}"
      loss = 2 - 2 * jnp.sum(pred * teacher, axis=-1)
      loss = jnp.mean(loss)
    return loss
  ...
  def apply_encoder(self, inputs, train):
    ...
    # masking: length -> length * mask_ratio
    # x, mask, ids_restore, ids_keep, ids_dump = random_mask(self.make_rng('dropout'), x, self.mask_ratio)
    ...
    # return x, mask, ids_restore, ids_keep, ids_dump
    return x
  ...
  def setup(self):
    ...
    if self.decoder.on_use:
      if self.decoder.cross_attention:
        ...
        if not self.use_prompt_decoder:
      ...
      decoder_layers['pred'] = t5x.layers.Dense(
        features=self.patches.size[0] * self.patches.size[1] * 3 if not self.use_teacher else self.teacher_dim,
        kernel_init=mlp_kernel_init,
        bias_init=mlp_bias_init,
...
class ImageLearner(nn.Module):
  ...
  def get_config_feat(self):
    cfg = self.config.model_img_feat.copy_and_resolve_references()  # copy
    cfg.name = 'img_feat'  # force name
    return cfg

  def setup(self):
    self.img_encoder = VisionTransformer(
      **self.get_config_img(), use_teacher=self.config.use_feat_target,
    )
    if self.config.use_feat_target:
      self.img_feat_extractor = ClipVisualBackbone(**self.get_config_feat())
    else:
      self.img_feat_extractor = None
  ...
  def __call__(self, inputs, *, train):
    ...
    # apply both encoders
    x_img, mask_img, ids_restore_img, ids_keep, ids_dump = self.img_encoder.apply_encoder(img, train=train)

    if self.img_feat_extractor:
      b, h, _, c = img.shape
      size = self.config.get("img_feat_resize_img", h)
      if size != h:
        resize_img = jax.image.resize(img, (b, size, size, c), "bicubic")
      else:
        resize_img = img
      x_feat = self.img_feat_extractor(resize_img, train=False)
      x_feat = jax.lax.stop_gradient(x_feat)
      logging.info("stop graident img feat extractor.")

      if self.config.get("pred_masked_only", False):
        x_feat = gather_by_einsum(x_feat[:, 1:, :], ids_dump)
    else:
      x_feat = None

    # print("encoder", x_img.shape, x_feat.shape)

    # apply both decoders
    x_img_full, x_img_part = self.img_encoder.apply_unshuffle(x_img, ids_restore_img)

    # print("shuffle", x_img_full.shape)


    if self.img_encoder.decoder.on_use:
      if self.img_encoder.use_prompt_decoder:
        use_cls_token = (self.img_encoder.classifier == 'token')
        num_clstokens = 1 if use_cls_token else 0

        fix_encoding_tokens = gather_by_einsum(x_img_full[:, num_clstokens:, :], ids_keep)
        mask_tokens = gather_by_einsum(x_img_full[:, num_clstokens:, :], ids_dump)
        fix_encoding_tokens = jnp.concatenate([x_img_full[:, :num_clstokens, :], fix_encoding_tokens], axis=1)
        # print(x_img_full.shape, fix_encoding_tokens.shape)
        pred_img = self.img_encoder.apply_decoder_prompt(
          mask_tokens, fix_encoding_tokens, train=train, ids_restore=ids_restore_img, pred_masked_only=self.config.get("pred_masked_only", False),
        )
      else:
        pred_img = self.img_encoder.apply_decoder(x_img_full, train=train)
        if self.config.get("pred_masked_only", False):
          pred_img = gather_by_einsum(pred_img[:, 1:, :], ids_dump)
    else:
      pred_img = None
    ...
    # compute losses
    if self.img_encoder.decoder.on_use:
      loss_img = self.img_encoder.compute_loss(img, pred_img, mask_img, teacher=x_feat)

      # remove cls token
      if self.img_feat_extractor and (self.img_encoder.classifier == 'token'):
        pred_img = pred_img[:, 1:, :]
      if not self.img_feat_extractor:
...
class ImageTextFeatLearner(nn.Module):
  ...
  def get_config_feat_img(self):
    cfg = self.config.model_img_feat.copy_and_resolve_references()  # copy
    cfg.name = 'img_encoder'  # force name
    return cfg

  def get_config_feat_txt(self):
    cfg = self.config.model_txt_feat.copy_and_resolve_references()  # copy
    cfg.name = 'txt_encoder'  # force name
    return cfg
  ...
  def setup(self):
    if self.config.clip_pretrain:
      self.img_encoder = ClipVisualBackbone(**self.get_config_feat_img())
      self.txt_encoder = ClipTextualBackbone(**self.get_config_feat_txt())
    else:
      self.img_encoder = VisionTransformer(**self.get_config_img(), use_teacher=self.config.use_feat_target)
  ...
  @nn.compact
  def __call__(self, inputs, *, train, encode_img=True, encode_txt=True, id=None):
    img = inputs['image']
    if img is None:
      z_img = None
    else:
      # apply both encoders
      if self.config.clip_pretrain:
        z_img = self.img_encoder(img, train=False)
        z_img = z_img[:, 0, :]
        z_img /= jnp.linalg.norm(z_img, axis=-1, keepdims=True)
      else:
        x_img = self.img_encoder.apply_encoder(img, train=False)
    ...
    if "txt" in inputs.keys():
      txt = inputs['txt']
      is_valid = inputs['txt_is_valid']

      def forward_txt(txt, is_valid):
        if self.config.clip_pretrain:
          z_txt = self.txt_encoder(txt, train=False)
          z_txt /= jnp.linalg.norm(z_txt, axis=-1, keepdims=True)
          return z_txt
        else:
          x_txt = self.txt_encoder.apply_encoder(txt, train=False)
          ...
          z_txt /= jnp.linalg.norm(z_txt, axis=-1, keepdims=True)
          return z_txt
      z_txt = forward_txt(txt, is_valid)
    else:
      z_txt = None

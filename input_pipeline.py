...
def preprocess_for_eval(image_bytes, dtype=tf.float32, image_size=None, aug=None):
  ...
  if aug is not None:
    ...
    crop_func = _decode_and_center_crop
  image = crop_func(image_bytes, image_size)
  image = tf.reshape(image, [image_size, image_size, 3])
  image = normalize_image(image)
  image = tf.image.convert_image_dtype(image, dtype=dtype)
  return image
...
def create_split(dataset_builder, batch_size, data_layout, train, dtype=tf.float32,
  ...
  if train:
    ...
    val_set = "validation" if "validation" in dataset_builder.info.splits else "test"
    validate_examples = dataset_builder.info.splits[val_set].num_examples
    logging.info(f"validate_examples {validate_examples} num_shards {num_shards}")
    split_size = validate_examples // num_shards
    start = shard_id * split_size
    split = '{}[{}:{}]'.format(val_set, start, start + split_size)
    # label_name = "label" if "label" in dataset_builder.info.features.keys() else "original_label"  # original_label or real_label
    if "label" in dataset_builder.info.features.keys():
      label_name = "label"
    elif 'real_label' in dataset_builder.info.features.keys():
      label_name = "real_label"  # original_label or real_label
    elif "labels" in dataset_builder.info.features.keys():
      label_name = "labels"
    elif "objects" in dataset_builder.info.features.keys():
      label_name = ("objects", "type")
  logging.info(f"==== dataset_builder.info.features {dataset_builder.info.features}")
  if type(label_name) == tuple:
    num_classes = dataset_builder.info.features[label_name[0]][label_name[1]].num_classes
  else:
    num_classes = dataset_builder.info.features[label_name].num_classes
  ...
  def decode_example(example):
    if type(label_name) == tuple:
      label = example[label_name[0]][label_name[1]]
    else:

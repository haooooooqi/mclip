...
def create_optimizer(config, params_names, steps_per_epoch):
  ...
  if config.opt_type in {'adamw', 'adarows'}:
    ...
    if config.model.use_feat_target:
      ...
      freeze_keys = config.model.get("freeze_keys", ["img_feat"])
    ...
    if config.get("learning_rate_factor", 1.0) != 1.0:
      lrf_func = lrd_util.lrf_func(config.get("learning_rate_factor_keys", []), config.learning_rate_factor)
      lrf = lrd_util.filter_parameters(params_names, lrf_func)
      # logging.info('Apply lrf: {}'.format(lrf))
      opt.optax_optimizer = optax._src.combine.chain(opt.optax_optimizer, lrd_util.scale_by_lrd(lrf))
